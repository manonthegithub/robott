...
- [x] Switched `cmd/robottt-mcp` transport from stdio to HTTP (`mcp.NewStreamableHTTPHandler`, `MCP_LISTEN_ADDR` default `:8081`) — realized mid-testing that stdio meant Claude Code (running on a separate dev machine) would need to SSH into the Pi to spawn the binary as a subprocess. Since the Pi is already reachable at a fixed network address for the REST API, HTTP transport lets `claude mcp add robottt --transport http http://<pi>:8081` connect directly, no SSH. **Not yet rebuilt/tested on the Pi** — needs `go build ./... && go test ./...` again; unverified assumption this round is `mcp.NewStreamableHTTPHandler`'s exact signature.
