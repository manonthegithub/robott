...
- [ ] **Not yet run: components 9+10 need `go generate ./... && go mod tidy && go build ./... && go test ./...` on the Pi.** Two fresh unverified assumptions this round: (a) oapi-codegen's `HandlerFromMux` function name/signature for std-http-server, (b) the entire `github.com/modelcontextprotocol/go-sdk` API surface (`mcp.NewServer`, `mcp.AddTool`, `mcp.CallToolResult`, `mcp.TextContent`, `mcp.StdioTransport`) and the generated enum type name `apigen.StepperRequestDir` for the `dir` field. Same pattern as component 8: expect a fix-up round from real compiler errors.
